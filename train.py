...
def train(prepared_train_labels, train_images_folder, num_refinement_stages, base_lr, batch_size, batches_per_iter,
    ...
    dataset = CocoTrainDataset(prepared_train_labels, train_images_folder,
                               ...
                               transform=transforms.Compose([
                                   ...
                                   Flip()]))
    train_loader = DataLoader(dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers, use_shared_memory=False)
    ...
    scheduler = optim.lr.MultiStepDecay(base_lr, milestones=drop_after_epoch, gamma=0.333)
    optimizer = optim.Adam(parameters=net.parameters(), learning_rate=scheduler, weight_decay=5e-4)
